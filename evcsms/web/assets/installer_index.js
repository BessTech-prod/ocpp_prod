...
(function(){
  ...
  // ------- Org-filter (endast portal_admin/admin/installer) -------
  let ROLE = null;
  ...
  async function initOrgFilterIfPortal(){
    ...
    // Dölj filter om inte portal/installer
  ...
  function renderGrid(cps, statusData){
    ...
    // Klientfilter för portal/installer
    let visible = cps.slice();
    const selectedOrg = ($('#orgFilter')?.value || '');
    if((ROLE==='portal_admin'||ROLE==='admin'||ROLE==='installer') && selectedOrg){
        ...
        <div class="card border-0 shadow-sm h-100">
          <div class="card-body">
            <h5 class="card-title d-flex align-items-center gap-2 mb-3">
              ...
              <span class="ms-1">${meta.alias || displayCpId(cpId)}</span>
              ${(ROLE==='portal_admin'||ROLE==='admin'||ROLE==='installer') && meta.org_id ? `<span class="badge bg-light text-muted border ms-auto">${meta.org_id}</span>` : ''}
            </h5>
